...
import {ContactRound,ExternalLink,KeyRound,RefreshCw,Search,ShieldCheck,UserPlus,Users} from 'lucide-react'
...
export default function ContactsPanel({dealers=[],selectedIds,onJob}){
  ...
  const visible=useMemo(()=>candidates.filter(c=>{
    const d=dealers.find(x=>x.dealer_id===c.dealer_id),hay=`${c.person_name} ${c.title} ${c.email||''} ${c.phone||''} ${d?.dealer_name||''}`.toLowerCase();return !query||hay.includes(query.toLowerCase())
  }),[candidates,dealers,query])

  async function reveal(){if(!selectedIds.length)return setMessage({type:'warning',text:'Select at least one dealer first.'});setBusy(true);setMessage(null);try{const created=await runDealerJob('CONTACTS',selectedIds,{concurrency:2,onProgress:x=>onJob?.(x.jobId)});onJob?.(created.jobs?.[0]?.job_id);await refresh();setMessage({type:'success',text:`Decision-maker discovery finished for ${selectedIds.length} dealer${selectedIds.length===1?'':'s'}.`})}catch(e){setMessage({type:'error',text:e.message})}finally{setBusy(false)}}
  ...
  return <section className="si-panel si-contact-panel">
    <div className="si-section-head"><div><p className="si-kicker">Decision makers</p><h2>Reveal public contacts, then review before CRM changes</h2><p className="di-sub">Public staff evidence is stored separately from Rickey's private book. New findings never overwrite an existing contact automatically.</p></div><Badge tone="good"><ShieldCheck size={13}/>REVIEW GATED</Badge></div>
    <div className="si-contact-actions">
      <button className="si-primary" onClick={reveal} disabled={busy||!selectedIds.length}><Users size={16}/>{busy?'Searching…':`Reveal contacts (${selectedIds.length})`}</button>
    ...
    {!visible.length&&<div className="si-empty"><Users/><h3>No public decision-maker candidates yet</h3><p>Select dealer(s), then run Reveal contacts.</p></div>}
