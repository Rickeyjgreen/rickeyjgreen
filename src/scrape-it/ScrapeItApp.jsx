    ...
    <main className="si-main">
      ...
      <section className="si-panel">
        ...
        <div className="si-grid si-grid-3">
          ...
          <article className="si-metric"><CheckCircle2/><span>Complete</span><strong>{state.stats.scanned_count||0}</strong><small>Current exhaustive public coverage</small></article>
          <article className="si-metric"><Truck/><span>Verified VINs</span><strong>{state.stats.vin_count||0}</strong><small>From latest safe COMPLETE snapshots</small></article>
        </div>
        <div className="si-control-bar">
          ...
          <button onClick={()=>choose('ALL')}>Select visible</button><button onClick={()=>choose('INCOMPLETE')}>Needs coverage</button><button onClick={()=>choose('FAILED')}>Hard errors</button><button onClick={()=>choose('CLEAR')}>Clear</button>
        </div>
        ...
        <div className="si-dealer-grid">{visible.map(d=>{const attempt=d.latest_attempt||d.latest_run,cov=d.current_coverage||{},snap=d.latest_snapshot,status=coverageStatus(d),attemptIssue=attempt&&attempt.status!=='COMPLETE'&&status==='COMPLETE';return <button key={d.dealer_id} className={`si-dealer-card ${selected.has(d.dealer_id)?'selected':''}`} onClick={()=>toggle(d.dealer_id)}>
          <div className="si-dealer-card-head"><span className="si-check">{selected.has(d.dealer_id)?'✓':''}</span><div><strong>{d.dealer_name}</strong><small>DLR #{d.dealer_id}</small></div><Badge tone={statusTone(status)}>{status}</Badge></div>
          <div className="si-dealer-card-body"><span><MapPin size={13}/>{d.city}, {d.state} · {d.region}</span><span><Globe2 size={13}/>{cov.platform||attempt?.platform||'Platform pending'}</span><span><SquareStack size={13}/>{snap?.vin_count??0} current proven VINs</span></div>
          {attemptIssue&&<small className="si-card-warning"><AlertTriangle size={12}/>Current coverage remains valid; latest attempt was {attempt.status}.</small>}
          {status!=='COMPLETE'&&attempt?.status==='ERROR'&&<small className="si-card-error"><XCircle size={12}/>{attempt.error_message}</small>}
          {status!=='COMPLETE'&&attempt?.status==='INCOMPLETE'&&<small className="si-card-warning"><AlertTriangle size={12}/>Observation captured; exhaustive coverage not proven.</small>}
        </button>})}</div>
      </section>

      <section className="si-panel">
        <div className="si-section-head"><div><p className="si-kicker">One-time investigation</p><h2>Scan a dealer outside the current book</h2><p className="di-sub">Paste a public dealer website. This probe does not add the rooftop to monitored dealers and does not alter Rickey's book.</p></div><Badge tone="idle">OBSERVATION ONLY</Badge></div>
        <div className="si-control-bar"><label><Globe2 size={15}/><input placeholder="https://www.exampledealer.com" value={probeUrl} onChange={e=>setProbeUrl(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')probeDealer()}}/></label><button className="si-primary" onClick={probeDealer} disabled={probeBusy}>{probeBusy?'Investigating…':'Investigate dealer'}</button></div>
        {probeMessage&&<div className={`di-message di-message-${probeMessage.type}`}>{probeMessage.text}</div>}
        {probeResult&&<div className="si-grid si-grid-3">
          <article className="si-metric"><Globe2/><span>Platform</span><strong>{probeResult.platform||'Unknown'}</strong><small>{probeResult.website}</small></article>
          <article className="si-metric"><Truck/><span>Validated VINs</span><strong>{probeResult.vin_count||0}</strong><small>{probeResult.pages_scanned||0} page(s) scanned</small></article>
          <article className="si-metric"><ShieldCheck/><span>Coverage</span><strong>{probeResult.coverage_status||'INCOMPLETE'}</strong><small>{probeResult.completeness_reason}</small></article>
        </div>}
        <div className="di-foot"><strong>Truth rule:</strong> One-time investigation remains public-web observation unless exhaustive coverage is proven. It does not create a monitored dealer record.</div>
      </section>
