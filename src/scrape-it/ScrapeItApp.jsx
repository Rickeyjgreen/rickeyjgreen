...
export default function ScrapeItApp() {
  ...
  return <div className="scrape-it-shell">
    ...
    <main className="si-main">
      <section className="si-hero"><div><p className="si-kicker">Thin vertical slice</p><h1>Source → observation → change → evidence → action.</h1><p>One real authoritative source, immutable raw capture, deterministic normalization, transparent change detection, and a recommendation that exposes what is known versus unknown.</p></div><div className="si-hero-metric"><span>Canonical rule</span><strong>No invented dealer need.</strong><p>Model-level safety evidence stays separate from VIN-level applicability and dealer willingness.</p></div></section>

      <section className="si-grid si-grid-3">
        <article className="si-metric"><Database /><span>Raw captures</span><strong>{stats.raw_records}</strong><small>Append-only PostgreSQL evidence</small></article>
        <article className="si-metric"><Activity /><span>Change events</span><strong>{stats.change_events}</strong><small>Baseline + meaningful diffs retained</small></article>
